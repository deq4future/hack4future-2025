...
# Modificada la URL i les dades de la petició
...
# Continuació del codi per al joc del penjat (mantenir-se igual)
penjat_dibuix = [
    """
     +---+
     |   |
...
while intents > 0:
    ...
    if len(entrada) > 1:
        if entrada_normalitzada == paraula_sense_accents:
            puntuacio += 50
            print("🎉 Has encertat la paraula sencera!")
            break
        else:
            intents -= 1
            puntuacio -= 20
            print(f"❌ No és la paraula correcta. Has perdut 20 punts. Intents restants: {intents}")
            continue

    if entrada_normalitzada in paraula_sense_accents:
        print(f"✅ Has encertat la lletra: {entrada}")
        lletres_endevinades.append(entrada_normalitzada)
        puntuacio += 10
    else:
    ...
    # Notificar el progrés després de cada intent
    notifica_partida(game_id, entrada, intents, puntuacio)

# Finalitza la partida al final del joc
